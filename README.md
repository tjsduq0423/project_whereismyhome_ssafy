...
### 프로젝트에 적용된 디자인 패턴
----
- Vue.js3 CompositionAPI를 프로젝트에서 활용했습니다. Vue의 Composition API는 JavaScript의 Proxy를 기반으로 동작합니다. 여기서 언급하는 "프록시"는 JavaScript의 Proxy 객체를 말합니다. 프록시 패턴은 객체 사이에 중간 계층을 추가하여 객체에 대한 접근을 제어하거나 변형하는 디자인 패턴입니다. 
Vue의 Composition API에서의 Proxy 객체는 프록시 패턴과 유사한 개념으로, 컴포넌트의 반응성을 관리하고 추적하는 역할을 수행합니다. Proxy 객체를 사용하여 컴포넌트의 데이터에 대한 접근을 가로채고, 변경을 감지하여 컴포넌트의 다시 렌더링을 트리거합니다.
이러한 Vue의 반응현 변수의 특징을 활용해 상태관리, 컴포저블등의 효율적인 코드를 통해 짧은 기간의 프로젝트에서 많은 시돌르 해볼 수 있었습니다.
----
- 클라이언트에 필요한 데이터를 관리하는 서버를 빌드하는데 SpringBoot 프레임워크르 사용했습니다. SpringBoot에는 의존성 주입 패턴 싱글톤 패턴 MVC 패턴 BeanFactory 패턴 등등.. 서버에서 자원을 관리하고 유지보수성을 고려한 디자인 패턴이 이미 많이 적용되어있습니다. 그 중 스프링시큐리티의 filter체인에서 Strategy 패턴은 애플리케이션의 인증,인가에 대한 설정을 동적으로 변경할 수 있도록 로직을 유연하게 분리해주고 로깅, 에러 핸들링, 트랜잭션 관리 등의 유지보수에 프록시 패턴이 적용된 AOP를 활용하여 관심사를 분리 코드의 재사용성과 유지보수성을 높였습니다. 또한 JPA를 활용할 때 빌더 패턴은 유연성과 유지보수성 높은 쿼리에 활용했습니다.
----
