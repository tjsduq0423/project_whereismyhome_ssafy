...
public interface CctvRepository extends JpaRepository<Cctv,Integer> {
    ...
    @Query("select new com.whereismyhome.amenities.dto.response.CctvResponseDto(c.id, c.usage, c.lat, c.lng) from Cctv c where st_contains(st_buffer(Point(:lng,:lat),0.01),c.localPoint)order by ST_Distance_Sphere(Point(:lng,:lat),c.localPoint) limit 500")
