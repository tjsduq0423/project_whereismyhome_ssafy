...
public interface HouseInfoRepository extends JpaRepository<HouseInfo, Long> {
    ...
    @Query("select new com.whereismyhome.houseinfo.dto.HouseResponseDto(h.aptCode,h.apartmentName,h.lng,h.lat) from HouseInfo h where st_distance_sphere(h.localPoint,Point(:lng,:lat)) <= 1000")
